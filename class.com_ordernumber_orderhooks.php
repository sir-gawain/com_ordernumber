...
class tx_com_ordernumber_orderhooks {
    ...
    function generateOrderId($orderId, $basket, $pobj) {
        ...
        if (isset($pobj->conf['tx_com_ordernumber.']['startingAt'])) {
            $this->minNumber = intval($pobj->conf['tx_com_ordernumber.']['startingAt']);
        }
        if (isset($pobj->conf['tx_com_ordernumber.']['endingAt'])) {
            $this->maxNumber = intval($pobj->conf['tx_com_ordernumber.']['endingAt']);
        }
        if (isset($pobj->conf['tx_com_ordernumber.']['interval'])) {
            $this->interval = intval($pobj->conf['tx_com_ordernumber.']['interval']);
        }
    ...
    function getNewId() {
        $newid = false;
        $this->tmpAbsFilename = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($this->tmpPath . $this->tmpFileprefix . $this->tmpFilename);
